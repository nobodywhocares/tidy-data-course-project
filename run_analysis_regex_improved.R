...
colNameFilterRegexes <- c("mean", "std")
...
## DRY priniciple inspired function to dynamically create the data column filter
## using specified regex patterns where the default is original "mean" and "std"
getColumnNameFilter <- function(dataTable, regexes = colNameFilterRegexes)
{
    colNameFltr <- character()
    for (regex in regexes)
    {
        colNameFltr <- c(colNameFltr,names(dataTable)[grep(regex,names(dataTable),ignore.case=TRUE)])
    }
    colNameFltr
}
...
filteredMasterDataTable <-masterDataTable[,c(subjectIdColName,activityIdColName,getColumnNameFilter(masterDataTable))]
...
write.table(masterDataCast,"./tidy_data_final_regex_configurable.txt", row.names=F)
